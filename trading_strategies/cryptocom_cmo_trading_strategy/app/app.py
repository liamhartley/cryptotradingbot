...
from trading_strategies.cryptocom_cmo_trading_strategy.config import LOGICAL_PARAMS
...
def lambda_handler(event, context):
    ...
    cryptocom_wrapper = CryptoComApi(
        api_key=os.getenv('CRYPTOCOM_API_KEY'),
        secret_key=os.getenv('CRYPTOCOM_API_SECRET'),
        time_offset=os.getenv('TIME_OFFSET')
    )
    ...
    assert LOGICAL_PARAMS['BASE_CURRENCY']+LOGICAL_PARAMS['QUOTE_CURRENCY'] == LOGICAL_PARAMS['PAIR']

    cryptocom_wrapper.get_public_trades(symbol='btcusdt')
    poloniex_pair = LOGICAL_PARAMS['QUOTE_CURRENCY'].upper()+'_'+LOGICAL_PARAMS['BASE_CURRENCY'].upper()
    cmo = poloniex_cmo_logic_no_pandas(pair=poloniex_pair, period=86400)

    # product_details = PublicClient().get_product_ticker(product_id=LOGICAL_PARAMS['PAIR'])
    product_details = cryptocom_wrapper.get_ticker(symbol=LOGICAL_PARAMS['PAIR'])

    # ask_price = float(product_details['ask'])
    ask_price = float(product_details['sell'])

    # TODO consider trade size instead of market order
    ...
    if cmo < LOGICAL_PARAMS["OVERSOLD_VALUE"]:
        response = cryptocom_wrapper.order(
            symbol=LOGICAL_PARAMS["PAIR"],
            side='BUY',
            order_type=2,
            volume=trade_size
        )
